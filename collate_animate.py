...
if __name__ == '__main__':
    ...
    for time in range (len(times)):
        ...
        for id in range(p):
            print("Reading grid data from ./output/output_" + str(id) + "_" + str(times[time]) + ".txt")
    ...
    fig = plt.figure()
    plt.imshow(all_grids[0, :, :], cmap='viridis', extent = [0, xmax, 0, ymax])
    plt.colorbar()
    plt.xlabel('x')
    plt.ylabel('y')
    ...
    def animate(i):
        plt.imshow(all_grids[i, :, :], cmap='viridis', extent = [0, xmax, 0, ymax])
